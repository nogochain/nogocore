...
import (
	...
	peerpkg "github.com/nogochain/nogocommons/peer"
...
func (sm *SyncManager) fetchHeaders() {
	...
	bestPeer.PushGetHeadersMsg(peerpkg.BlockLocator(locator), &zeroHash)
...
func (sm *SyncManager) startSync() {
	...
	//
	// Peers are ranked by their reported block height. A future optimization
	// may incorporate latency metrics and enable parallel sync from multiple peers.
...
func (sm *SyncManager) clearRequestedState(state *peerSyncState) {
	...
	// Removing requested blocks from the global map so they will be
	// fetched from other peers on the next inventory round.
...
func (sm *SyncManager) handleBlockMsg(bmsg *blockMsg) {
	...
	if isOrphan {
		...
		if err != nil {
			...
			peer.PushGetBlocksMsg(peerpkg.BlockLocator(locator), orphanRoot)
		}
	} else {
		if peer == sm.syncPeer {
			sm.lastProgressTime = time.Now()
...
func (sm *SyncManager) handleHeadersMsg(hmsg *headersMsg) {
	...
	if sm.ibdMode {
		...
		if bestHeight < sm.syncPeer.LastBlock() {
			...
			sm.syncPeer.PushGetHeadersMsg(peerpkg.BlockLocator(locator), &zeroHash)
			return
		}
	}
	...
	for i, iv := range invVects {
		...
		if iv.Type == wire.InvTypeBlock {
			...
			if i == lastBlock {
				...
				peer.PushGetBlocksMsg(peerpkg.BlockLocator(locator), &zeroHash)
			}
		}
	}
...
func (sm *SyncManager) handleBlockchainNotification(notification *blockchain.Notification) {
	...
	case blockchain.NTBlockAccepted:
		...
		block, ok := notification.Data.(*nogoutil.Block)
		if !ok {
...
// processed.
func (sm *SyncManager) QueueTx(tx *nogoutil.Tx, peer *peerpkg.Peer, done chan struct{}) {
...
// processed.
func (sm *SyncManager) QueueBlock(block *nogoutil.Block, peer *peerpkg.Peer, done chan struct{}) {
...
// chain.
func (sm *SyncManager) ProcessBlock(block *nogoutil.Block, flags blockchain.BehaviorFlags) (bool, error) {
