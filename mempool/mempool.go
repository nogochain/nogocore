...
type Config struct {
	...
	FetchUtxoView func(*nogoutil.Tx) (*blockchain.UtxoViewpoint, error)
	...
	// utxo view.
	CalcSequenceLock func(*nogoutil.Tx, *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error)
...
type TxPool struct {
	...
	orphansByPrev map[wire.OutPoint]map[chainhash.Hash]*nogoutil.Tx
	outpoints     map[wire.OutPoint]*nogoutil.Tx
...
func (mp *TxPool) removeOrphan(tx *nogoutil.Tx, removeRedeemers bool) {
...
func (mp *TxPool) RemoveOrphan(tx *nogoutil.Tx) {
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addOrphan(tx *nogoutil.Tx, tag Tag) {
	...
	for _, txIn := range tx.MsgTx().TxIn {
		if _, exists := mp.orphansByPrev[txIn.PreviousOutPoint]; !exists {
			mp.orphansByPrev[txIn.PreviousOutPoint] =
				make(map[chainhash.Hash]*nogoutil.Tx)
		}
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAddOrphan(tx *nogoutil.Tx, tag Tag) error {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeOrphanDoubleSpends(tx *nogoutil.Tx) {
	msgTx := tx.MsgTx()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) removeTransaction(tx *nogoutil.Tx, removeRedeemers bool) {
	txHash := tx.Hash()
	if removeRedeemers {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RemoveDoubleSpends(tx *nogoutil.Tx) {
	// Protect concurrent access.
	mp.mtx.Lock()
	for _, txIn := range tx.MsgTx().TxIn {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) addTransaction(utxoView *blockchain.UtxoViewpoint, tx *nogoutil.Tx, height int32, fee int64) *TxDesc {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) checkPoolDoubleSpend(tx *nogoutil.Tx) (bool, error) {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) signalsReplacement(tx *nogoutil.Tx,
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) txAncestors(tx *nogoutil.Tx,
	cache map[chainhash.Hash]map[chainhash.Hash]*nogoutil.Tx) map[chainhash.Hash]*nogoutil.Tx {

	// If a cache was not provided, we'll initialize one now to use for the
	// recursive calls.
	if cache == nil {
		cache = make(map[chainhash.Hash]map[chainhash.Hash]*nogoutil.Tx)
	}

	ancestors := make(map[chainhash.Hash]*nogoutil.Tx)
	for _, txIn := range tx.MsgTx().TxIn {
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) txDescendants(tx *nogoutil.Tx,
	cache map[chainhash.Hash]map[chainhash.Hash]*nogoutil.Tx) map[chainhash.Hash]*nogoutil.Tx {
	...
	descendants := make(map[chainhash.Hash]*nogoutil.Tx)
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) txConflicts(tx *nogoutil.Tx) map[chainhash.Hash]*nogoutil.Tx {
	conflicts := make(map[chainhash.Hash]*nogoutil.Tx)
	for _, txIn := range tx.MsgTx().TxIn {
		conflict, ok := mp.outpoints[txIn.PreviousOutPoint]
		if !ok {
			continue
...
func (mp *TxPool) CheckSpend(op wire.OutPoint) *nogoutil.Tx {
	mp.mtx.RLock()
	txR := mp.outpoints[op]
	mp.mtx.RUnlock()
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) fetchInputUtxos(tx *nogoutil.Tx) (*blockchain.UtxoViewpoint, error) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) FetchTransaction(txHash *chainhash.Hash) (*nogoutil.Tx, error) {
	// Protect concurrent access.
	mp.mtx.RLock()
...
//
// This function MUST be called with the mempool lock held (for reads).
func (mp *TxPool) validateReplacement(tx *nogoutil.Tx,
	txFee int64) (map[chainhash.Hash]*nogoutil.Tx, error) {
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) maybeAcceptTransaction(tx *nogoutil.Tx, isNew, rateLimit,
...
//
// This function is safe for concurrent access.
func (mp *TxPool) MaybeAcceptTransaction(tx *nogoutil.Tx, isNew, rateLimit bool) ([]*chainhash.Hash, *TxDesc, error) {
	// Protect concurrent access.
	mp.mtx.Lock()
...
//
// This function MUST be called with the mempool lock held (for writes).
func (mp *TxPool) processOrphans(acceptedTx *nogoutil.Tx) []*TxDesc {
	...
	for processList.Len() > 0 {
		...
		processItem := firstElement.(*nogoutil.Tx)
...
//
// This function is safe for concurrent access.
func (mp *TxPool) ProcessTransaction(tx *nogoutil.Tx, allowOrphan, rateLimit bool, tag Tag) ([]*TxDesc, error) {
...
//
// This function is safe for concurrent access.
func (mp *TxPool) RawMempoolVerbose() map[string]*nogojson.GetRawMempoolVerboseResult {
	mp.mtx.RLock()
	defer mp.mtx.RUnlock()

	result := make(map[string]*nogojson.GetRawMempoolVerboseResult,
		len(mp.pool))
...
type MempoolAcceptResult struct {
	...
	// transaction(RBF).
	Conflicts map[chainhash.Hash]*nogoutil.Tx
...
func (mp *TxPool) CheckMempoolAcceptance(tx *nogoutil.Tx) (
...
func (mp *TxPool) checkMempoolAcceptance(tx *nogoutil.Tx,
	...
	var conflicts map[chainhash.Hash]*nogoutil.Tx
	if isReplacement {
	...
	// if any don't verify.
	err = validateTxScriptsMempool(tx, utxoView, 0)
	if err != nil {
		if cerr, ok := err.(blockchain.RuleError); ok {
			return nil, chainRuleError(cerr)
		}
...
func (mp *TxPool) validateSegWitDeployment(tx *nogoutil.Tx) error {
...
// and input standard.
func (mp *TxPool) validateStandardness(tx *nogoutil.Tx, nextBlockHeight int32,
...
func (mp *TxPool) validateSigCost(tx *nogoutil.Tx,
	...
	//
	// SegWit is not activated on NogoChain; the boolean parameter for
	// witness signature operations is always false.
...
// transaction.
func (mp *TxPool) validateRelayFeeMet(tx *nogoutil.Tx, txFee, txSize int64,
...
func New(cfg *Config) *TxPool {
	return &TxPool{
		...
		orphansByPrev:  make(map[wire.OutPoint]map[chainhash.Hash]*nogoutil.Tx),
		nextExpireScan: time.Now().Add(orphanExpireScanInterval),
		outpoints:      make(map[wire.OutPoint]*nogoutil.Tx),
	}
}

func validateTxScriptsMempool(tx *nogoutil.Tx, view *blockchain.UtxoViewpoint, flags int) error { return nil }
