...
func TestCalcMinRequiredTxRelayFee(t *testing.T) {
	tests := []struct {
		...
		relayFee nogoutil.Amount // minimum relay transaction fee.
...
func TestDust(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wire.TxOut{Value: nogoutil.MaxSatoshi, PkScript: pkScript},
			nogoutil.MaxSatoshi,
			false,
		},
		{
...
func TestCheckTransactionStandard(t *testing.T) {
	...
	for _, test := range tests {
		...
		err := CheckTransactionStandard(
			nogoutil.NewTx(&test.tx), test.height, pastMedianTime,
