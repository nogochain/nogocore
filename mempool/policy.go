...
const (
	...
	DefaultMinRelayTxFee = nogoutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee nogoutil.Amount) int64 {
	...
	if minFee < 0 || minFee > nogoutil.MaxSatoshi {
		minFee = nogoutil.MaxSatoshi
	}
...
// accurately and concisely via the ScriptVerifyCleanStack and
// ScriptVerifySigPushOnly flags.
func checkInputsStandard(tx *nogoutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
// utxoView interface.
func checkInputsStandardWithView(tx *nogoutil.Tx, utxoView utxoView) error {
	...
	for i, txIn := range tx.MsgTx().TxIn {
		...
		if IsPayToAnchorScript(originPkScript) {
		...
		switch GetScriptClass(originPkScript) {
		case ScriptHashTy:
...
// public keys.
func checkPkScriptStandard(pkScript []byte, scriptClass ScriptClass) error {
	switch scriptClass {
	case MultiSigTy:
...
func IsDust(txOut *wire.TxOut, minRelayTxFee nogoutil.Amount) bool {
...
func CheckTransactionStandard(tx *nogoutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee nogoutil.Amount,
	...
	for i, txIn := range msgTx.TxIn {
		...
		if !IsPushOnlyScript(txIn.SignatureScript) {
			str := fmt.Sprintf("transaction input %d: signature "+
	...
	for i, txOut := range msgTx.TxOut {
		scriptClass := GetScriptClass(txOut.PkScript)
...
func GetTxVirtualSize(tx *nogoutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
...
type ScriptClass int; const NonStandardTy ScriptClass = 0; const ScriptHashTy ScriptClass = 1; const MultiSigTy ScriptClass = 2; func GetScriptClass(script []byte) ScriptClass { return NonStandardTy }; func GetPreciseSigOpCount(a, b []byte, c bool) int { return 0 }; func CalcMultiSigStats(script []byte) (int, int, error) { return 0, 0, nil }; func IsPayToAnchorScript(script []byte) bool { return false }

func IsWitnessProgram(script []byte) bool { return false }; func IsUnspendable(script []byte) bool { return false }; func IsPushOnlyScript(script []byte) bool { return false }; const NullDataTy ScriptClass = 3
