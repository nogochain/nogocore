...
type TxMempool interface {
	...
	RawMempoolVerbose() map[string]*nogojson.GetRawMempoolVerboseResult
	...
	FetchTransaction(txHash *chainhash.Hash) (*nogoutil.Tx, error)
	...
	ProcessTransaction(tx *nogoutil.Tx, allowOrphan,
	...
	// orphans.
	RemoveTransaction(tx *nogoutil.Tx, removeRedeemers bool)
	...
	// actions based on it.
	CheckMempoolAcceptance(tx *nogoutil.Tx) (*MempoolAcceptResult, error)
	...
	CheckSpend(op wire.OutPoint) *nogoutil.Tx
}
