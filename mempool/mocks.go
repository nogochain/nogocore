...
func (m *MockTxMempool) RawMempoolVerbose() map[string]*nogojson.
	GetRawMempoolVerboseResult {

	args := m.Called()
	return args.Get(0).(map[string]*nogojson.GetRawMempoolVerboseResult)
}
...
func (m *MockTxMempool) FetchTransaction(
	txHash *chainhash.Hash) (*nogoutil.Tx, error) {
	...
	return args.Get(0).(*nogoutil.Tx), args.Error(1)
}
...
func (m *MockTxMempool) ProcessTransaction(tx *nogoutil.Tx, allowOrphan,
...
func (m *MockTxMempool) RemoveTransaction(tx *nogoutil.Tx,
...
func (m *MockTxMempool) CheckMempoolAcceptance(
	tx *nogoutil.Tx) (*MempoolAcceptResult, error) {
...
func (m *MockTxMempool) CheckSpend(op wire.OutPoint) *nogoutil.Tx {
	args := m.Called(op)
