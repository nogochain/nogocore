...
// Fee estimation is based on Gavin Andresen's initial model with observed
// confirmation data grouped by fee rate. Extensions proposed by Alex Morcos
// (e.g. decaying historical weight) are candidates for future improvement.
// Reference: https://lists.linuxfoundation.org/pipermail/bitcoin-dev/2014-October/006824.html
...
// the given fee rate.
func (rate SatoshiPerByte) Fee(size uint32) nogoutil.Amount {
	// If our rate is the error value, return that.
	if rate == SatoshiPerByte(-1) {
		return nogoutil.Amount(-1)
	}

	return nogoutil.Amount(float64(rate) * float64(size))
}

// NewSatoshiPerByte creates a SatoshiPerByte from an Amount and a
// size in bytes.
func NewSatoshiPerByte(fee nogoutil.Amount, size uint32) SatoshiPerByte {
...
func (ef *FeeEstimator) ObserveTransaction(t *TxDesc) {
	...
	if _, ok := ef.observed[hash]; !ok {
		...
		ef.observed[hash] = &observedTransaction{
			hash:     hash,
			feeRate:  NewSatoshiPerByte(nogoutil.Amount(t.Fee), size),
			observed: t.Height,
...
func (ef *FeeEstimator) RegisterBlock(block *nogoutil.Block) error {
	ef.mtx.Lock()
	defer ef.mtx.Unlock()
	...
	transactions := make(map[*nogoutil.Tx]struct{})
	...
	for t := range transactions {
		...
		//
		// Transactions that exceed the estimation depth or have negative
		// confirmation counts are excluded. A more accurate estimator could
		// incorporate partial-weighting rather than skipping these transactions.
	...
	for i, j := range replacementCounters {
		for {
			...
			if prev.mined == ef.lastKnownHeight {
				...
				// Setting the removed element to nil would prevent retaining
				// a reference but causes a panic due to slice length mismatch
				// with the append-based removal pattern used here.
				ef.bin[i] = newBin
...
func (ef *FeeEstimator) Save() FeeEstimatorState {
	...
	// The serialization buffer starts at zero capacity and grows as needed;
	// a pre-allocated capacity could be computed from the number of fee bins.
