...
func (s *fakeChain) FetchUtxoView(tx *nogoutil.Tx) (*blockchain.UtxoViewpoint, error) {
	s.RLock()
	defer s.RUnlock()
...
func (s *fakeChain) CalcSequenceLock(tx *nogoutil.Tx,
...
// transactions.
func txOutToSpendableOut(tx *nogoutil.Tx, outputNum uint32) spendableOutput {
	return spendableOutput{
		outPoint: wire.OutPoint{Hash: *tx.Hash(), Index: outputNum},
		amount:   nogoutil.Amount(tx.MsgTx().TxOut[outputNum].Value),
	}
}
...
// version 2 blocks.
func (p *poolHarness) CreateCoinbaseTx(blockHeight int32, numOutputs uint32) (*nogoutil.Tx, error) {
...
func (p *poolHarness) CreateSignedTx(inputs []spendableOutput,
	numOutputs uint32, fee nogoutil.Amount,
	signalsReplacement bool) (*nogoutil.Tx, error) {

	// Calculate the total input amount and split it amongst the requested
	// number of outputs.
	var totalInput nogoutil.Amount
...
func (p *poolHarness) CreateTxChain(firstOutput spendableOutput, numTxns uint32) ([]*nogoutil.Tx, error) {
	txChain := make([]*nogoutil.Tx, 0, numTxns)
	...
	for i := uint32(0); i < numTxns; i++ {
		...
		txChain = append(txChain, nogoutil.NewTx(tx))
...
func newPoolHarness(chainParams *chaincfg.Params) (*poolHarness, []spendableOutput, error) {
	...
	signKey, signPub := nogoec.PrivKeyFromBytes(keyBytes)
...
// mock chain.
func (ctx *testContext) addCoinbaseTx(numOutputs uint32) *nogoutil.Tx {
	ctx.t.Helper()
...
func (ctx *testContext) addSignedTx(inputs []spendableOutput,
	numOutputs uint32, fee nogoutil.Amount,
	signalsReplacement, confirmed bool) *nogoutil.Tx {
...
func testPoolMembership(tc *testContext, tx *nogoutil.Tx, inOrphanPool, inTxPool bool) {
	tc.t.Helper()
...
func TestBasicOrphanRemoval(t *testing.T) {
	...
	nonChainedOrphanTx, err := harness.CreateSignedTx([]spendableOutput{{
		amount:   nogoutil.Amount(5000000000),
...
func TestSignalsReplacement(t *testing.T) {
	...
	testCases := []struct {
		name               string
		setup              func(ctx *testContext) *nogoutil.Tx
...
func TestConflicts(t *testing.T) {
	...
	testCases := []struct {
		...
		setup func(ctx *testContext) (*nogoutil.Tx, []*nogoutil.Tx)
	}{
		{
		...
		},
		{
			...
			setup: func(ctx *testContext) (*nogoutil.Tx, []*nogoutil.Tx) {
				...
				return tx, []*nogoutil.Tx{conflict1, conflict2}
			},
		},
		{
			// Create a transaction that would introduce two
			...
			setup: func(ctx *testContext) (*nogoutil.Tx, []*nogoutil.Tx) {
				...
				return tx, []*nogoutil.Tx{parent, child}
			},
		},
	}
...
func TestRBF(t *testing.T) {
	...
	const defaultFee = nogoutil.SatoshiPerBitcoin
	...
	testCases := []struct {
		...
		},
		{
			...
			setup: func(ctx *testContext) (*nogoutil.Tx, []*nogoutil.Tx) {
				...
				return childReplace, []*nogoutil.Tx{childNoReplace}
			},
			err: "",
		},
	}
