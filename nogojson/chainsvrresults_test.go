...
func TestChainSvrCustomResults(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result: &nogojson.GetZmqNotificationResult{{
				Type: "pubrawblock",
...
func TestGetTxOutSetInfoResult(t *testing.T) {
	...
	tests := []struct {
		name   string
		result string
		want   nogojson.GetTxOutSetInfoResult
	}{
		{
			...
			want: nogojson.GetTxOutSetInfoResult{
				...
				DiskSize: 1,
				TotalAmount: func() nogoutil.Amount {
					a, err := nogoutil.NewAmount(0.2)
					if err != nil {
						panic(err)
					}
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		var out nogojson.GetTxOutSetInfoResult
...
func TestChainSvrMiningInfoResults(t *testing.T) {
	...
	tests := []struct {
		name     string
		result   string
		expected nogojson.GetMiningInfoResult
	}{
		{
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		var miningInfoResult nogojson.GetMiningInfoResult
...
func TestStringOrArrayMarshalJSON(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "single element",
			input:    nogojson.StringOrArray{"test"},
			expected: []byte(`["test"]`),
		},
		{
			name:     "multiple elements",
			input:    nogojson.StringOrArray{"test", "test1"},
			expected: []byte(`["test","test1"]`),
		},
		{
			name:     "element requiring escaping",
			input:    nogojson.StringOrArray{`a "quoted" value`},
...
func TestStringOrArrayRoundTrip(t *testing.T) {
	...
	for _, in := range inputs {
		...
		var out nogojson.StringOrArray
...
func TestGetBlockChainInfoWarnings(t *testing.T) {
	...
	tests := []struct {
		name     string
		result   string
		expected nogojson.StringOrArray
	}{
		{
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			var info nogojson.GetBlockChainInfoResult
...
func TestGetNetworkInfoWarnings(t *testing.T) {
	...
	tests := []struct {
		name     string
		result   string
		expected nogojson.GetNetworkInfoResult
	}{
		{
			name:   "network info with single warning",
			result: `{"warnings": "this is a warning"}`,
			expected: nogojson.GetNetworkInfoResult{
				Warnings: nogojson.StringOrArray{
					"this is a warning",
				},
			},
		},
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		var infoResult nogojson.GetNetworkInfoResult
