...
func TestAssignField(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			dest:     nogojson.Int32(0),
			src:      "null",
			expected: nil,
		},
		{
			name:     `null optional field - "null" -> *string`,
			dest:     nogojson.String(""),
			src:      "null",
			expected: nil,
		},
	}
	...
	for i, test := range tests {
		...
		err := nogojson.TstAssignField(1, "testField", dst, src)
		if err != nil {
...
func TestNewCmdErrors(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			method: "boguscommand",
			args:   []interface{}{},
			err:    nogojson.Error{ErrorCode: nogojson.ErrUnregisteredMethod},
		},
		{
			name:   "too few parameters to command with required + optional",
			method: "getblock",
			args:   []interface{}{},
			err:    nogojson.Error{ErrorCode: nogojson.ErrNumParams},
		},
		{
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		_, err := nogojson.NewCmd(test.method, test.args...)
		if reflect.TypeOf(err) != reflect.TypeOf(test.err) {
...
func TestMarshalCmd(t *testing.T) {
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		bytes, err := nogojson.MarshalCmd(nogojson.RpcVersion1, test.id, test.cmd)
		if err != nil {
			t.Errorf("Test #%d (%s) wrong error - got %T (%v)",
				i, test.name, err, err)
			continue
...
func TestMarshalCmdErrors(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			id:   1,
			cmd:  (*nogojson.GetBlockCmd)(nil),
			err:  nogojson.Error{ErrorCode: nogojson.ErrInvalidType},
		},
		{
			name: "nil instance of registered type",
			id:   []int{0, 1},
			cmd:  &nogojson.GetBlockCountCmd{},
			err:  nogojson.Error{ErrorCode: nogojson.ErrInvalidType},
		},
	}
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		_, err := nogojson.MarshalCmd(nogojson.RpcVersion1, test.id, test.cmd)
		if reflect.TypeOf(err) != reflect.TypeOf(test.err) {
			t.Errorf("Test #%d (%s) wrong error - got %T (%v), "+
				"want %T", i, test.name, err, err, test.err)
			continue
...
func TestUnmarshalCmdErrors(t *testing.T) {
	...
	tests := []struct {
		name    string
		request nogojson.Request
		err     nogojson.Error
	}{
		{
			name: "unregistered type",
			request: nogojson.Request{
				Jsonrpc: nogojson.RpcVersion1,
				Method:  "bogusmethod",
				Params:  nil,
				ID:      nil,
			},
	...
	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		_, err := nogojson.UnmarshalCmd(&test.request)
		if reflect.TypeOf(err) != reflect.TypeOf(test.err) {
			t.Errorf("Test #%d (%s) wrong error - got %T (%v), "+
				"want %T", i, test.name, err, err, test.err)
			continue
