...
func TestUsageFlagStringer(t *testing.T) {
	...
	tests := []struct {
		in   nogojson.UsageFlag
		want string
	}{
		{0, "0x0"},
		{nogojson.UFWalletOnly, "UFWalletOnly"},
		{nogojson.UFWebsocketOnly, "UFWebsocketOnly"},
		{nogojson.UFNotification, "UFNotification"},
		{nogojson.UFWalletOnly | nogojson.UFWebsocketOnly,
			"UFWalletOnly|UFWebsocketOnly"},
		{nogojson.UFWalletOnly | nogojson.UFWebsocketOnly | (1 << 31),
			"UFWalletOnly|UFWebsocketOnly|0x80000000"},
	}

	// Detect additional usage flags that don't have the stringer added.
	numUsageFlags := 0
	highestUsageFlagBit := nogojson.TstHighestUsageFlagBit
...
func TestRegisterCmdErrors(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			err: nogojson.Error{ErrorCode: nogojson.ErrDuplicateMethod},
		},
		{
			...
			flags: nogojson.TstHighestUsageFlagBit,
			err:   nogojson.Error{ErrorCode: nogojson.ErrInvalidUsageFlags},
		},
		{
			name:   "invalid type",
			method: "registertestcmd",
		...
		},
		{
			...
			err: nogojson.Error{ErrorCode: nogojson.ErrEmbeddedType},
		},
		{
			...
			err: nogojson.Error{ErrorCode: nogojson.ErrUnexportedField},
		},
		{
			...
			err: nogojson.Error{ErrorCode: nogojson.ErrUnsupportedFieldType},
		},
		{
		...
		},
		{
			...
			err: nogojson.Error{ErrorCode: nogojson.ErrNonOptionalField},
		},
		{
			...
			err: nogojson.Error{ErrorCode: nogojson.ErrNonOptionalDefault},
		},
		{
			...
			err: nogojson.Error{ErrorCode: nogojson.ErrMismatchedDefault},
		},
	}

	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		err := nogojson.RegisterCmd(test.method, test.cmdFunc(),
			test.flags)
...
func TestMustRegisterCmdPanic(t *testing.T) {
	...
	nogojson.MustRegisterCmd("panicme", 0, 0)
}
...
func TestRegisteredCmdMethods(t *testing.T) {
	...
	methods := nogojson.RegisteredCmdMethods()
	if len(methods) == 0 {
