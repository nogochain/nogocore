...
func TestSubmitPackageCmd(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			unmarshalled: &nogojson.JsonSubmitPackageCmd{
				RawTxs:        []string{"hex1", "hex2"},
				MaxFeeRate:    nogojson.Float64(0.1),
				MaxBurnAmount: nil,
			},
		},
		{
			...
			unmarshalled: &nogojson.JsonSubmitPackageCmd{
				...
				MaxFeeRate:    nogojson.Float64(0.25),
				MaxBurnAmount: nogojson.Float64(0.001),
			},
		},
		{
		...
		},
		{
			...
			unmarshalled: &nogojson.JsonSubmitPackageCmd{
				RawTxs:        []string{"hex1", "hex2"},
				MaxFeeRate:    nogojson.Float64(0.0),
				MaxBurnAmount: nil,
			},
		},
	}
	...
	for i, test := range tests {
		...
		// creation function.
		marshalled, err := nogojson.MarshalCmd(
			nogojson.RpcVersion1, testID, test.staticCmd(),
		)
		require.NoError(
		...
		// creation function.
		marshalled, err = nogojson.MarshalCmd(
			nogojson.RpcVersion1, testID, cmd,
		)
		require.NoError(
			t, err, "MarshalCmd #%d (%s) unexpected in test", i,
			test.name,
...
func TestSubmitPackageResultUnmarshalJSON(t *testing.T) {
	...
	tests := []struct {
		name     string
		json     string
		expected nogojson.SubmitPackageResult
		wantErr  bool
	}{
		{
			...
			expected: nogojson.SubmitPackageResult{
				PackageMsg: "success",
				TxResults: map[string]nogojson.SubmitPackageTxResult{
					"wtxid1": {
		...
		},
		{
			...
			expected: nogojson.SubmitPackageResult{
				...
				TxResults: map[string]nogojson.SubmitPackageTxResult{
					"wtxid1": {
						TxID:  *mustParseHash("1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef"),
						Error: nogojson.String("insufficient fee"),
					},
				},
			},
		},
