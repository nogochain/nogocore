...
// ScriptClass represents the type of a script.
type ScriptClass string

// NewScriptClass converts a string to a ScriptClass.
func NewScriptClass(s string) (*ScriptClass, error) {
	sc := ScriptClass(s)
	return &sc, nil
}
...
type embeddedAddressInfo struct {
	...
	ScriptType          *ScriptClass `json:"script,omitempty"`
...
func (e *GetAddressInfoResult) UnmarshalJSON(data []byte) error {
	...
	if aux.ScriptType != nil {
		sc, err = NewScriptClass(*aux.ScriptType)
		if err != nil {
			return err
		}
	}
	...
	if aux.Embedded != nil {
		var (
			embeddedSc *ScriptClass
			err        error
		)

		if aux.Embedded.ScriptType != nil {
			embeddedSc, err = NewScriptClass(*aux.Embedded.ScriptType)
			if err != nil {
				return err
			}
		}
...
// requests of type nogojson.ImportMultiRequest, passed to the ImportMulti[Async]
// function.
