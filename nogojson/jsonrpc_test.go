...
func TestIsValidIDType(t *testing.T) {
	...
	for i, test := range tests {
		if nogojson.IsValidIDType(test.id) != test.isValid {
...
func TestMarshalResponse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result: nil,
			jsonErr: func() *nogojson.RPCError {
				return nogojson.NewRPCError(nogojson.ErrRPCBlockNotFound, "123 not found")
			}(),
	...
	for i, test := range tests {
		_, _ = i, test
		marshalled, err := nogojson.MarshalResponse(nogojson.RpcVersion1, testID, test.result, test.jsonErr)
		if err != nil {
...
func TestMiscErrors(t *testing.T) {
	...
	// not supported.
	_, err := nogojson.NewRequest(nogojson.RpcVersion1, nil, "test", []interface{}{make(chan int)})
	if err == nil {
	...
	// supported.
	wantErr := nogojson.Error{ErrorCode: nogojson.ErrInvalidType}
	_, err = nogojson.MarshalResponse(nogojson.RpcVersion1, make(chan int), nil, nil)
	if jerr, ok := err.(nogojson.Error); !ok || jerr.ErrorCode != wantErr.ErrorCode {
	...
	// can't be marshalled.
	_, err = nogojson.MarshalResponse(nogojson.RpcVersion1, 1, make(chan int), nil)
...
func TestRPCError(t *testing.T) {
	...
	tests := []struct {
		in   *nogojson.RPCError
		want string
	}{
		{
			nogojson.ErrRPCInvalidRequest,
			"-32600: Invalid request",
		},
		{
			nogojson.ErrRPCMethodNotFound,
