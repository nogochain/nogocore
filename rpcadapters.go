...
// rpcserverPeer provides an interface for peer information needed by the RPC
// server. Defined here to break the circular dependency with rpcserver.go.
...
// rpcserverConnManager provides an interface for connection management needed
// by the RPC server.
...
// rpcserverSyncManager provides an interface for sync management needed by the
// RPC server.
type rpcserverSyncManager interface {
	IsCurrent() bool
	SubmitBlock(block *nogoutil.Block, flags blockchain.BehaviorFlags) (bool, error)
	Pause() chan<- struct{}
	SyncPeerID() int32
...
// This function is safe for concurrent access and is part of the
// rpcserverSyncManager interface implementation.
func (b *rpcSyncMgr) SubmitBlock(block *nogoutil.Block, flags blockchain.BehaviorFlags) (bool, error) {
