...
// defaultHomeDir returns the OS-appropriate home directory for NogoCore.
func defaultHomeDir() string {
	// Search for Windows APPDATA first.
	appData := os.Getenv("APPDATA")
	if appData != "" {
		return filepath.Join(appData, "nogocore")
	}

	// Fall back to standard HOME directory for POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".nogocore")
	}

	return "."
}

// oldHomeDir returns the OS-specific home directory used prior to
// version 1.0.0. This has since been replaced with defaultHomeDir, but
// this function is still provided for the automatic upgrade path.
func oldHomeDir() string {
...
// location prior to version 1.0.0 and uses heuristics to ascertain the old
// database type to rename to the new format.
func upgradeDBPathNet(oldDbPath, netName string) error {
	// Prior to version 1.0.0, the database was named the same for
	// both sqlite and leveldb. Use heuristics to figure out the type
	// of the database and move it to the new path and name accordingly.
...
// version 1.0.0 to their new locations.
func upgradeDBPaths() error {
	// Prior to version 1.0.0, the databases were in the "db" directory and
	...
	// new path accordingly.
	oldDbRoot := filepath.Join(oldHomeDir(), "db")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "nogocore.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "nogocore_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "nogocore_regtest.db"), "regtest")
...
// doUpgrades performs upgrades to nogocore as new versions require it.
