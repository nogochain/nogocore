...
func (c *Client) trackRegisteredNtfns(cmd interface{}) {
	...
	case *nogojson.NotifyBlocksCmd:
		c.ntfnState.notifyBlocks = true

	case *nogojson.NotifyNewTransactionsCmd:
...
type IndividualBulkResult struct {
	...
	Error  *nogojson.RPCError `json:"error"`
	Id     uint64            `json:"id"`
}
...
// returning an unmarshaled nogojson.RPCError (or an unmarshalling error) if so.
...
func (c *Client) reregisterNtfns() error {
	...
	if nslen > 0 {
		outpoints := make([]nogojson.OutPoint, 0, nslen)
...
func (c *Client) SendCmd(cmd interface{}) chan *Response {
	rpcVersion := nogojson.RpcVersion1
	if c.batch {
		rpcVersion = nogojson.RpcVersion2
	}
	// Get the method associated with the command.
	method, err := nogojson.CmdMethod(cmd)
	if err != nil {
		return newFutureError(err)
	}
	...
	// Marshal the command.
	id := c.NextID()
	marshalledJSON, err := nogojson.MarshalCmd(rpcVersion, id, cmd)
	if err != nil {
		return newFutureError(err)
	}
...
func (c *Client) BackendVersion() (BackendVersion, error) {
	...
	case *nogojson.RPCError:
		if err.Code != nogojson.ErrRPCMethodNotFound.Code {
