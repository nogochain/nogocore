...
type notificationState struct {
	...
	notifySpent        map[nogojson.OutPoint]struct{}
}
...
func (s *notificationState) Copy() *notificationState {
	...
	stateCopy.notifySpent = make(map[nogojson.OutPoint]struct{})
...
func newNotificationState() *notificationState {
	return &notificationState{
		...
		notifySpent:    make(map[nogojson.OutPoint]struct{}),
	}
}
...
type NotificationHandlers struct {
	...
	OnRecvTx func(transaction *nogoutil.Tx, details *nogojson.BlockDetails)
	...
	//
	// Deprecated: Use OnRelevantTxAccepted instead.
	OnRedeemingTx func(transaction *nogoutil.Tx, details *nogojson.BlockDetails)
	...
	// made to register for the notification and the function is non-nil.
	OnTxAccepted func(hash *chainhash.Hash, amount nogoutil.Amount)

	// OnTxAccepted is invoked when a transaction is accepted into the
	// memory pool.  It will only be invoked if a preceding call to
	// NotifyNewTransactions with the verbose flag set to true has been
	// made to register for the notification and the function is non-nil.
	OnTxAcceptedVerbose func(txDetails *nogojson.TxRawResult)
	...
	// such as btcwallet.
	OnAccountBalance func(account string, balance nogoutil.Amount, confirmed bool)
...
func (c *Client) handleNotification(ntfn *rawNotification) {
	...
	switch ntfn.Method {
	// OnBlockConnected
	case nogojson.BlockConnectedNtfnMethod:
	...
	case nogojson.FilteredBlockConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnBlockDisconnected
	case nogojson.BlockDisconnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case nogojson.FilteredBlockDisconnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRecvTx
	case nogojson.RecvTxNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRedeemingTx
	case nogojson.RedeemingTxNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	case nogojson.RelevantTxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRescanFinished
	case nogojson.RescanFinishedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnRescanProgress
	case nogojson.RescanProgressNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAccepted
	case nogojson.TxAcceptedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnTxAcceptedVerbose
	case nogojson.TxAcceptedVerboseNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnBtcdConnected
	case nogojson.BtcdConnectedNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnAccountBalance
	case nogojson.AccountBalanceNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
	...
	// OnWalletLockState
	case nogojson.WalletLockStateNtfnMethod:
		// Ignore the notification if the client is not interested in
		// it.
...
func parseFilteredBlockConnectedParams(params []json.RawMessage) (int32,
	*wire.BlockHeader, []*nogoutil.Tx, error) {
	...
	transactions := make([]*nogoutil.Tx, len(hexTransactions))
	for i, hexTx := range hexTransactions {
		...
		transactions[i], err = nogoutil.NewTxFromBytes(transaction)
		if err != nil {
			return 0, nil, nil, err
		}
	}
...
// notifications.
func parseChainTxNtfnParams(params []json.RawMessage) (*nogoutil.Tx,
	*nogojson.BlockDetails, error) {
	...
	// JSON object.
	var block *nogojson.BlockDetails
	if len(params) > 1 {
	...
	return nogoutil.NewTx(&msgTx), block, nil
}
...
func parseTxAcceptedNtfnParams(params []json.RawMessage) (*chainhash.Hash,
	...
	// Bounds check amount.
	amt, err := nogoutil.NewAmount(famt)
	if err != nil {
		return nil, 0, err
	}
...
func parseTxAcceptedVerboseNtfnParams(params []json.RawMessage) (*nogojson.TxRawResult,
	error) {
	...
	var rawTx nogojson.TxRawResult
...
func parseAccountBalanceNtfnParams(params []json.RawMessage) (account string,
	balance nogoutil.Amount, confirmed bool, err error) {
	...
	// Bounds check amount.
	bal, err := nogoutil.NewAmount(fbal)
	if err != nil {
		return "", 0, false, err
	}
...
func (c *Client) NotifyBlocksAsync() FutureNotifyBlocksResult {
	...
	cmd := nogojson.NewNotifyBlocksCmd()
...
func (c *Client) notifySpentInternal(outpoints []nogojson.OutPoint) FutureNotifySpentResult {
	// Not supported in HTTP POST mode.
	if c.config.HTTPPostMode {
		return newFutureError(ErrWebsocketsRequired)
	}
	...
	cmd := nogojson.NewNotifySpentCmd(outpoints)
	return c.SendCmd(cmd)
}

// newOutPointFromWire constructs the btcjson representation of a transaction
// outpoint from the wire type.
func newOutPointFromWire(op *wire.OutPoint) nogojson.OutPoint {
	return nogojson.OutPoint{
		Hash:  op.Hash.String(),
		Index: op.Index,
	}
}
...
func (c *Client) NotifySpentAsync(outpoints []*wire.OutPoint) FutureNotifySpentResult {
	...
	ops := make([]nogojson.OutPoint, 0, len(outpoints))
	for _, outpoint := range outpoints {
		ops = append(ops, newOutPointFromWire(outpoint))
	}
	cmd := nogojson.NewNotifySpentCmd(ops)
	return c.SendCmd(cmd)
}
...
func (c *Client) NotifyNewTransactionsAsync(verbose bool) FutureNotifyNewTransactionsResult {
	...
	cmd := nogojson.NewNotifyNewTransactionsCmd(&verbose)
	return c.SendCmd(cmd)
}
...
func (c *Client) notifyReceivedInternal(addresses []string) FutureNotifyReceivedResult {
	...
	cmd := nogojson.NewNotifyReceivedCmd(addresses)
	return c.SendCmd(cmd)
}
...
func (c *Client) NotifyReceivedAsync(
	...
	cmd := nogojson.NewNotifyReceivedCmd(addrs)
	return c.SendCmd(cmd)
}
...
func (c *Client) RescanAsync(startBlock *chainhash.Hash,
	...
	cmd := nogojson.NewRescanCmd(startBlockHashStr, addrs, ops, nil)
	return c.SendCmd(cmd)
}
...
func (c *Client) RescanEndBlockAsync(startBlock *chainhash.Hash,
	...
	cmd := nogojson.NewRescanCmd(startBlockHashStr, addrs, ops,
		&endBlockHashStr)
	return c.SendCmd(cmd)
}
...
func (c *Client) LoadTxFilterAsync(reload bool, addresses []address.Address,
	...
	outPointObjects := make([]nogojson.OutPoint, len(outPoints))
	for i := range outPoints {
		outPointObjects[i] = nogojson.OutPoint{
			Hash:  outPoints[i].Hash.String(),
			Index: outPoints[i].Index,
		}
	}

	cmd := nogojson.NewLoadTxFilterCmd(reload, addrStrs, outPointObjects)
	return c.SendCmd(cmd)
}
