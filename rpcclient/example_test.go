	...
	requests := []nogojson.ImportMultiRequest{
		{
			...
			Range:     &nogojson.DescriptorRange{Value: []int{0, 100}},
			Timestamp: nogojson.TimestampOrNow{Value: 0}, // scan from genesis
			WatchOnly: nogojson.Bool(true),
			KeyPool:   nogojson.Bool(false),
			Internal:  nogojson.Bool(false),
		},
	}
	opts := &nogojson.ImportMultiOptions{Rescan: true}
