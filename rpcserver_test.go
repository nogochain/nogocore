...
func TestHandleTestMempoolAcceptFailDecode(t *testing.T) {
	...
	testCases := []struct {
		name            string
		txns            []string
		expectedErrCode nogojson.RPCErrorCode
	}{
		{
			name:            "hex decode fail",
			txns:            []string{"invalid"},
			expectedErrCode: nogojson.ErrRPCDecodeHexString,
		},
		{
			name:            "tx decode fail",
			txns:            []string{"696e76616c6964"},
			expectedErrCode: nogojson.ErrRPCDeserialization,
		},
	}
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			cmd := nogojson.NewTestMempoolAcceptCmd(tc.txns, 0)
			...
			require.Error(err)
			rpcErr, ok := err.(*nogojson.RPCError)
			require.True(ok)
...
func decodeTxHex(t *testing.T, txHex string) *nogoutil.Tx {
	rawBytes, err := hex.DecodeString(txHex)
	require.NoError(t, err)
	tx, err := nogoutil.NewTxFromBytes(rawBytes)
	require.NoError(t, err)
...
func TestHandleTestMempoolAcceptMixedResults(t *testing.T) {
	...
	expectedResults := make([]*nogojson.TestMempoolAcceptResult, 3)
	...
	// error.
	expectedResults[0] = &nogojson.TestMempoolAcceptResult{
	...
	expectedResults[1] = &nogojson.TestMempoolAcceptResult{
	...
	const feeSats = nogoutil.Amount(1000)
	...
	expectedResults[2] = &nogojson.TestMempoolAcceptResult{
		Txid:    tx3.Hash().String(),
		Wtxid:   tx3.WitnessHash().String(),
		Allowed: true,
		Vsize:   100,
		Fees: &nogojson.TestMempoolAcceptFees{
			Base:             feeSats.ToBTC(),
	...
	cmd := nogojson.NewTestMempoolAcceptCmd(
...
func TestValidateFeeRate(t *testing.T) {
	...
	const (
		...
		testFeeSats = nogoutil.Amount(testFeeRate * 1e5 * testTxSize)
	)

	testCases := []struct {
		name         string
		feeSats      nogoutil.Amount
		txSize       int64
		maxFeeRate   float64
		expectedFees *nogojson.TestMempoolAcceptFees
		allowed      bool
	}{
		{
...
func TestHandleTestMempoolAcceptFees(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// details.
			expected := &nogojson.TestMempoolAcceptResult{
				Txid:    tx.Hash().String(),
			...
			if tc.allowed {
				...
				expected.Fees = &nogojson.TestMempoolAcceptFees{
					Base:             feeSats / 1e8,
			...
			results, ok := r.([]*nogojson.TestMempoolAcceptResult)
			require.True(ok)
...
func TestGetTxSpendingPrevOut(t *testing.T) {
	...
	cmd := &nogojson.GetTxSpendingPrevOutCmd{
	...
	expectedResults := []*nogojson.GetTxSpendingPrevOutResult{
		{
