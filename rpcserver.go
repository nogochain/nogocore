...
import (
	...
	"github.com/nogochain/nogocommons/nogoec/ecdsa"
...
var (
	...
	// data.
	gbtCoinbaseAux = &nogojson.GetBlockTemplateResultAux{
...
var (
	...
	ErrRPCUnimplemented = &nogojson.RPCError{
		Code:    nogojson.ErrRPCUnimplemented,
		Message: "Command unimplemented",
	}

	// ErrRPCNoWallet is an error returned to RPC clients when the provided
	// command is recognized as a wallet command.
	ErrRPCNoWallet = &nogojson.RPCError{
		Code:    nogojson.ErrRPCNoWallet,
...
// not needed.
func internalRPCError(errStr, context string) *nogojson.RPCError {
	logStr := errStr
	if context != "" {
		logStr = context + ": " + errStr
	}
	rpcsLog.Error(logStr)
	return nogojson.NewRPCError(nogojson.ErrRPCInternal.Code, errStr)
}

// rpcDecodeHexError is a convenience function for returning a nicely formatted
// RPC error which indicates the provided hex string failed to decode.
func rpcDecodeHexError(gotHex string) *nogojson.RPCError {
	return nogojson.NewRPCError(nogojson.ErrRPCDecodeHexString,
...
// transaction hash.
func rpcNoTxInfoError(txHash *chainhash.Hash) *nogojson.RPCError {
	return nogojson.NewRPCError(nogojson.ErrRPCNoTxInfo,
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*nogojson.CreateRawTransactionCmd)
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount*nogoutil.SatoshiPerBitcoin > nogoutil.MaxSatoshi {
			return nil, &nogojson.RPCError{
				Code:    nogojson.ErrRPCType,
		...
		satoshi, err := nogoutil.NewAmount(amount)
		if err != nil {
...
// transaction.
func createVinList(mtx *wire.MsgTx) []nogojson.Vin {
	// Coinbase transactions only have a single txin by definition.
	vinList := make([]nogojson.Vin, len(mtx.TxIn))
	...
	for i, txIn := range mtx.TxIn {
		...
		vinEntry.ScriptSig = &nogojson.ScriptSig{
			Asm: disbuf,
...
// transaction.
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []nogojson.Vout {
	voutList := make([]nogojson.Vout, 0, len(mtx.TxOut))
	for i, v := range mtx.TxOut {
		...
		var vout nogojson.Vout
		vout.N = uint32(i)
		vout.Value = nogoutil.Amount(v.Value).ToBTC()
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx,
	...
	blkHeight int32, chainHeight int32) (*nogojson.TxRawResult, error) {
	...
	txReply := &nogojson.TxRawResult{
		Hex:      mtxHex,
		Txid:     txHash,
		Hash:     mtx.WitnessHash().String(),
		Size:     int32(mtx.SerializeSize()),
		Vsize:    int32(mempool.GetTxVirtualSize(nogoutil.NewTx(mtx))),
		Weight:   int32(blockchain.GetTransactionWeight(nogoutil.NewTx(mtx))),
...
func handleDecodeRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*nogojson.DecodeRawTransactionCmd)
	...
	if err != nil {
		return nil, &nogojson.RPCError{
			Code:    nogojson.ErrRPCDeserialization,
			Message: "TX decode failed: " + err.Error(),
		}
	}

	// Create and return the result.
	txReply := nogojson.TxRawDecodeResult{
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*nogojson.DecodeScriptCmd)
	...
	reply := nogojson.DecodeScriptResult{
		Asm:       disbuf,
...
// CPU mining (cpuminer) was removed per NogoCore Phase 2.
func handleGenerate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	return nil, &nogojson.RPCError{
		Code:    nogojson.ErrRPCInternal.Code,
		Message: "CPU mining removed. Use nogominer (solo) or nogopool for mining.",
	}
}

// handleGetAddedNodeInfo handles getaddednodeinfo commands.
func handleGetAddedNodeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*nogojson.GetAddedNodeInfoCmd)
	...
	if c.Node != nil {
		...
		if !found {
			return nil, &nogojson.RPCError{
				Code:    nogojson.ErrRPCClientNodeNotAdded,
	...
	results := make([]*nogojson.GetAddedNodeInfoResult, 0, len(peers))
	for _, rpcPeer := range peers {
		// Set the "address" of the peer which could be an ip address
		// or a domain name.
		peer := rpcPeer.ToPeer()
		var result nogojson.GetAddedNodeInfoResult
		result.AddedNode = peer.Addr()
		result.Connected = nogojson.Bool(peer.Connected())
		...
		addrs := make([]nogojson.GetAddedNodeInfoResultAddr, 0, len(ipList))
		for _, ip := range ipList {
			var addr nogojson.GetAddedNodeInfoResultAddr
			addr.Address = ip
...
func handleGetBestBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	result := &nogojson.GetBestBlockResult{
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	blk, err := nogoutil.NewBlockFromBytes(blkBytes)
	if err != nil {
	...
	blockReply := nogojson.GetBlockVerboseResult{
		Hash:          c.Hash,
	...
	if *c.Verbosity == 1 {
		...
		rawTxns := make([]nogojson.TxRawResult, len(txns))
		for i, tx := range txns {
...
func handleGetBlockChainInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	chainInfo := &nogojson.GetBlockChainInfoResult{
		Chain:         params.Name,
		...
		Pruned:        false, // NogoCore does not support pruning.
		SoftForks: &nogojson.SoftForks{
			Bip9SoftForks: make(map[string]*nogojson.Bip9SoftForkDescription),
		},
	}
	...
	chainInfo.SoftForks.SoftForks = []*nogojson.SoftForkDescription{
		{
			ID:      "bip34",
			Version: 2,
			Reject: struct {
	...
	for deployment, deploymentDetails := range params.Deployments {
		...
		// Finally, populate the soft-fork description.
		// NogoCore uses simplified deployment model without MedianTime.
		var startTime, endTime int64
		chainInfo.SoftForks.Bip9SoftForks[forkName] = &nogojson.Bip9SoftForkDescription{
...
func handleGetBlockHash(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*nogojson.GetBlockHashCmd)
	hash, err := s.cfg.Chain.BlockHashByHeight(int32(c.Index))
	if err != nil {
		return nil, &nogojson.RPCError{
			Code:    nogojson.ErrRPCOutOfRange,
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*nogojson.GetBlockHeaderCmd)
	...
	params := s.cfg.ChainParams
	blockHeaderReply := nogojson.GetBlockHeaderVerboseResult{
		Hash:          c.Hash,
		Confirmations: int64(1 + best.Height - blockHeight),
		Height:        blockHeight,
		Version:       blockHeader.Version,
...
func (state *gbtWorkState) updateBlockTemplate(s *rpcServer, useCoinbaseValue bool) error {
	...
	if template == nil || state.prevHash == nil ||
		...
		if !useCoinbaseValue {
			addrStr := cfg.MiningAddrs[rand.Intn(len(cfg.MiningAddrs))]
			payAddr, _ = address.NewAddressPubKeyHash(address.Hash160([]byte(addrStr)), s.cfg.ChainParams)
		}
		...
		if !useCoinbaseValue && !template.ValidPayAddress {
			...
			payAddrStr := cfg.MiningAddrs[rand.Intn(len(cfg.MiningAddrs))]
			payAddr, _ := address.DecodeAddress(payAddrStr, s.cfg.ChainParams)
			...
			block := nogoutil.NewBlock(template.Block)
...
// state as a nogojson.GetBlockTemplateResult that is ready to be encoded to JSON
// and returned to the caller.
//
// This function MUST be called with the state locked.
func (state *gbtWorkState) blockTemplateResult(useCoinbaseValue bool, submitOld *bool) (*nogojson.GetBlockTemplateResult, error) {
	...
	transactions := make([]nogojson.GetBlockTemplateResultTx, 0, numTx-1)
	...
	for i, tx := range msgBlock.Transactions {
		...
		bTx := nogoutil.NewTx(tx)
		resultTx := nogojson.GetBlockTemplateResultTx{
	...
	reply := nogojson.GetBlockTemplateResult{
...
func handleGetBlockTemplateRequest(s *rpcServer, request *nogojson.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
	...
	// simulation test mode, or when mining addresses are configured
	// (solo mining / --miningaddr set, aligned with btcd --generate).
	name := activeNetParams.Name
	if name != "regtest" && name != "simnet" &&
		len(cfg.MiningAddrs) == 0 &&
		s.cfg.ConnMgr.ConnectedCount() == 0 {

		return nil, &nogojson.RPCError{
			Code:    nogojson.ErrRPCClientNotConnected,
	...
	if currentHeight != 0 && !s.cfg.SyncMgr.IsCurrent() {
		return nil, &nogojson.RPCError{
			Code:    nogojson.ErrRPCClientInInitialDownload,
...
func handleGetBlockTemplateProposal(s *rpcServer, request *nogojson.TemplateRequest) (interface{}, error) {
	hexData := request.Data
	if hexData == "" {
		return false, &nogojson.RPCError{
			Code: nogojson.ErrRPCType,
...
func handleGetBlockTemplate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*nogojson.GetBlockTemplateCmd)
	request := c.Request
...
func handleGetChainTips(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := make([]nogojson.GetChainTipsResult, 0, len(chainTips))
...
func handleGetCFilter(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	if s.cfg.CfIndex == nil {
		return nil, &nogojson.RPCError{
			Code:    nogojson.ErrRPCNoCFIndex,
			Message: "The CF index must be enabled for this command",
		}
	}

	c := cmd.(*nogojson.GetCFilterCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetCFilterHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*nogojson.GetCFilterHeaderCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &nogojson.InfoChainResult{
		...
		TestNet:         activeNetParams.Name == "testnet",
		RelayFee:        float64(cfg.minRelayTxFee) / 1e8,
	}
...
func handleGetMempoolInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &nogojson.GetMempoolInfoResult{
...
func handleGetMiningInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	gnhpsCmd := nogojson.NewGetNetworkHashPSCmd(nil, nil)
	...
	best := s.cfg.Chain.BestSnapshot()
	result := nogojson.GetMiningInfoResult{
...
func handleGetNetTotals(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	reply := &nogojson.GetNetTotalsResult{
...
func handleGetNetworkHashPS(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*nogojson.GetNetworkHashPSCmd)
...
func handleGetNodeAddresses(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*nogojson.GetNodeAddressesCmd)
	...
	addresses := make([]*nogojson.GetNodeAddressesResult, 0, count)
	for _, node := range nodes[:count] {
		address := &nogojson.GetNodeAddressesResult{
...
func handleGetPeerInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	infos := make([]*nogojson.GetPeerInfoResult, 0, len(peers))
	for _, p := range peers {
		statsSnap := p.ToPeer().StatsSnapshot()
		info := &nogojson.GetPeerInfoResult{
			ID:             statsSnap.ID,
			Addr:           statsSnap.Addr,
...
func handleGetRawMempool(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*nogojson.GetRawMempoolCmd)
	mp := s.cfg.TxMemPool
...
func handleGetRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*nogojson.GetRawTransactionCmd)
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	// Note: accessing the mempool transaction map is subject to a race
	// condition with mempool removal; the error on fetch handles the
	// edge case where the transaction is removed between check and fetch.
	if includeMempool && s.cfg.TxMemPool.HaveTransaction(txHash) {
		...
		if c.Vout > uint32(len(mtx.TxOut)-1) {
			return nil, &nogojson.RPCError{
				Code: nogojson.ErrRPCInvalidTxVout,
	...
	txOutReply := &nogojson.GetTxOutResult{
		BestBlock:     bestBlockHash,
		Confirmations: int64(confirmations),
		Value:         nogoutil.Amount(value).ToBTC(),
		ScriptPubKey: nogojson.ScriptPubKeyResult{
			Asm:       disbuf,
...
func handleInvalidateBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*nogojson.InvalidateBlockCmd)
...
// passed transaction.
func createVinListPrevOut(s *rpcServer, mtx *wire.MsgTx, chainParams *chaincfg.Params, vinExtra bool, filterAddrMap map[string]struct{}) ([]nogojson.VinPrevOut, error) {
	// Coinbase transactions only have a single txin by definition.
	if blockchain.IsCoinBaseTx(mtx) {
		...
		txIn := mtx.TxIn[0]
		vinList := make([]nogojson.VinPrevOut, 1)
		vinList[0].Coinbase = hex.EncodeToString(txIn.SignatureScript)
		vinList[0].Sequence = txIn.Sequence
		return vinList, nil
	}
	...
	vinList := make([]nogojson.VinPrevOut, 0, len(mtx.TxIn))
	...
	for _, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &nogojson.PrevOut{
				Addresses: encodedAddrs,
				Value:     nogoutil.Amount(originTxOut.Value).ToBTC(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr address.Address, numToSkip,
	numRequested uint32) ([]*nogoutil.Tx, uint32) {
...
func handleReconsiderBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*nogojson.ReconsiderBlockCmd)
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*nogojson.SearchRawTransactionsCmd)
	vinExtra := false
	if c.VinExtra != nil {
	...
	// The verbose flag is set, so generate the JSON object and return it.
	best := s.cfg.Chain.BestSnapshot()
	srtList := make([]nogojson.SearchRawTransactionsResult, len(addressTxns))
	for i := range addressTxns {
...
func handleSendRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*nogojson.SendRawTransactionCmd)
	...
	if err != nil {
		...
		if txRuleErr, ok := ruleErr.Err.(mempool.TxRuleError); ok {
			...
			case strings.Contains(
				...
				code = nogojson.ErrRPCTxAlreadyInChain

			default:
				code = nogojson.ErrRPCTxRejected
			}
		}
...
func handleSignMessageWithPrivKey(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*nogojson.SignMessageWithPrivKeyCmd)

	wif, err := nogoutil.DecodeWIF(c.PrivKey)
	if err != nil {
		message := "Invalid private key"
		switch err {
		case nogoutil.ErrMalformedPrivateKey:
...
// The miner submits a complete, self-contained serialized block (header +
// all transactions). The block is deserialized and validated as-is against
// the blockchain consensus rules — there is no cached template involved in
// validation. This follows the standard btcd architecture where the block
// stands on its own.
func handleSubmitBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*nogojson.SubmitBlockCmd)
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*nogojson.ValidateAddressCmd)

	result := nogojson.ValidateAddressChainResult{}
	...
	case *address.AddressPubKeyHash:
		result.IsScript = nogojson.Bool(false)
		result.IsWitness = nogojson.Bool(false)

	case *address.AddressScriptHash:
		result.IsScript = nogojson.Bool(true)
		result.IsWitness = nogojson.Bool(false)
	...
	case *address.AddressWitnessPubKeyHash:
		result.IsScript = nogojson.Bool(false)
		result.IsWitness = nogojson.Bool(true)
		result.WitnessVersion = nogojson.Int32(int32(addr.WitnessVersion()))
		result.WitnessProgram = nogojson.String(hex.EncodeToString(addr.WitnessProgram()))
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*nogojson.VerifyMessageCmd)
...
func handleVersion(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	result := map[string]nogojson.VersionResult{
		"btcdjsonrpcapi": {
...
func handleTestMempoolAccept(s *rpcServer, cmd interface{},
	...
	c := cmd.(*nogojson.TestMempoolAcceptCmd)

	// Create txns to hold the decoded tx.
	txns := make([]*nogoutil.Tx, 0, len(c.RawTxns))
	...
	results := make([]*nogojson.TestMempoolAcceptResult, 0, len(txns))
	for _, tx := range txns {
		// Create a test result item.
		item := &nogojson.TestMempoolAcceptResult{
			Txid:  tx.Hash().String(),
		...
		// Errors from individual package validation are stored as the
		// reject reason; future refinement may split this into distinct
		// error categories for better RPC diagnostics.
...
func handleGetTxSpendingPrevOut(s *rpcServer, cmd interface{},
	...
	c := cmd.(*nogojson.GetTxSpendingPrevOutCmd)
	...
	results := make([]*nogojson.GetTxSpendingPrevOutResult, 0, len(ops))
	for _, op := range ops {
		// Create a result entry.
		result := &nogojson.GetTxSpendingPrevOutResult{
			Txid: op.Hash.String(),
			Vout: op.Index,
		}
...
func validateFeeRate(feeSats nogoutil.Amount, txSize int64,
	maxFeeRate float64) (*nogojson.TestMempoolAcceptFees, bool) {

	// Calculate fee rate in sats/kvB.
	feeRateSatsPerKVB := feeSats * 1e3 / nogoutil.Amount(txSize)
	...
	return &nogojson.TestMempoolAcceptFees{
		Base:             feeSats.ToBTC(),
...
type parsedRPCCmd struct {
	jsonrpc nogojson.RPCVersion
	id      interface{}
	method  string
	cmd     interface{}
	err     *nogojson.RPCError
}
...
func (s *rpcServer) standardCmdResult(cmd *parsedRPCCmd, closeChan <-chan struct{}) (interface{}, error) {
	...
	return nil, nogojson.ErrRPCMethodNotFound
handled:
...
func parseCmd(request *nogojson.Request) *parsedRPCCmd {
	...
	if err != nil {
		...
		if jerr, ok := err.(nogojson.Error); ok &&
			jerr.ErrorCode == nogojson.ErrUnregisteredMethod {

			parsedCmd.err = nogojson.ErrRPCMethodNotFound
			return &parsedCmd
		}

		// Otherwise, some type of invalid parameters is the
		// cause, so produce the equivalent RPC error.
		parsedCmd.err = nogojson.NewRPCError(
			nogojson.ErrRPCInvalidParams.Code, err.Error())
		return &parsedCmd
	}
...
// the type *nogojson.RPCError to the appropriate type as needed.
func createMarshalledReply(rpcVersion nogojson.RPCVersion, id interface{}, result interface{}, replyErr error) ([]byte, error) {
	var jsonErr *nogojson.RPCError
	if replyErr != nil {
		if jErr, ok := replyErr.(*nogojson.RPCError); ok {
			jsonErr = jErr
		} else {
	...
	return nogojson.MarshalResponse(rpcVersion, id, result, jsonErr)
}

// processRequest determines the incoming request type (single or batched),
// parses it and returns a marshalled response.
func (s *rpcServer) processRequest(request *nogojson.Request, isAdmin bool, closeChan <-chan struct{}) []byte {
	...
	if jsonErr == nil {
		...
		if parsedCmd.err != nil {
			...
			result, err = s.standardCmdResult(parsedCmd,
				closeChan)
			if err != nil {
				if rpcErr, ok := err.(*nogojson.RPCError); ok {
					jsonErr = rpcErr
				} else {
					jsonErr = &nogojson.RPCError{
						Code:    nogojson.ErrRPCInvalidRequest.Code,
...
func (s *rpcServer) jsonRPCRead(w http.ResponseWriter, r *http.Request, isAdmin bool) {
	...
	if !batchedRequest {
		...
		if err != nil {
			...
			resp, err = nogojson.MarshalResponse(nogojson.RpcVersion1, nil, nil, jsonErr)
			if err != nil {
	...
	if batchedRequest {
		...
		if err != nil {
			...
			resp, err = nogojson.MarshalResponse(nogojson.RpcVersion2, nil, nil, jsonErr)
			if err != nil {
				rpcsLog.Errorf("Failed to create reply: %v", err)
			}
			...
			if len(batchedRequests) > 0 {
				...
				for _, entry := range batchedRequests {
					...
					if err != nil {
						...
						resp, err = nogojson.MarshalResponse("", nil, nil, jsonErr)
						if err != nil {
							rpcsLog.Errorf("Failed to create reply: %v", err)
						}
...
type rpcserverConfig struct {
	...
	// Generator produces block templates for getblocktemplate and mining.
	// CPU mining (cpuminer) was removed per NogoCore Phase 2 — mining is
	// performed externally via nogominer or nogopool.
