...
import (
	...
	"github.com/nogochain/nogocommons/txscript"
...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *nogoutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *nogoutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *nogoutil.Tx, isNew bool) {
...
// Notification types
type notificationBlockConnected nogoutil.Block
type notificationBlockDisconnected nogoutil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *nogoutil.Tx,
...
func (*wsNotificationManager) notifyBlockConnected(clients map[chan struct{}]*wsClient,
	block *nogoutil.Block) {

	// Notify interested websocket clients about the connected block.
	ntfn := nogojson.NewBlockConnectedNtfn(block.Hash().String(), block.Height(),
		block.MsgBlock().Header.Timestamp.Unix())
	marshalledJSON, err := nogojson.MarshalCmd(nogojson.RpcVersion1, nil, ntfn)
	if err != nil {
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *nogoutil.Block) {
	...
	ntfn := nogojson.NewBlockDisconnectedNtfn(block.Hash().String(),
...
func (m *wsNotificationManager) notifyFilteredBlockConnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := nogojson.NewFilteredBlockConnectedNtfn(block.Height(),
...
func (*wsNotificationManager) notifyFilteredBlockDisconnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := nogojson.NewFilteredBlockDisconnectedNtfn(block.Height(),
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *nogoutil.Tx) {
	...
	ntfn := nogojson.NewTxAcceptedNtfn(txHashStr, nogoutil.Amount(amount).ToBTC())
	marshalledJSON, err := nogojson.MarshalCmd(nogojson.RpcVersion1, nil, ntfn)
	if err != nil {
	...
	var verboseNtfn *nogojson.TxAcceptedVerboseNtfn
	...
	for _, wsc := range clients {
		if wsc.verboseTxUpdates {
			...
			verboseNtfn = nogojson.NewTxAcceptedVerboseNtfn(*rawTx)
			marshalledJSONVerbose, err = nogojson.MarshalCmd(nogojson.RpcVersion1, nil,
				verboseNtfn)
			if err != nil {
...
func (m *wsNotificationManager) addSpentRequests(opMap map[wire.OutPoint]map[chan struct{}]*wsClient,
	...
	spends := make(map[chainhash.Hash]*nogoutil.Tx)
	for _, op := range ops {
...
func blockDetails(block *nogoutil.Block, txIndex int) *nogojson.BlockDetails {
	if block == nil {
		return nil
	}
	return &nogojson.BlockDetails{
		Height: block.Height(),
...
func newRedeemingTxNotification(txHex string, index int, block *nogoutil.Block) ([]byte, error) {
	// Create and marshal the notification.
	ntfn := nogojson.NewRedeemingTxNtfn(txHex, blockDetails(block, index))
	return nogojson.MarshalCmd(nogojson.RpcVersion1, nil, ntfn)
}
...
func (m *wsNotificationManager) notifyForTxOuts(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	addrs map[string]map[chan struct{}]*wsClient, tx *nogoutil.Tx, block *nogoutil.Block) {
	...
	for i, txOut := range tx.MsgTx().TxOut {
		...
		for _, txAddr := range txAddrs {
			...
			ntfn := nogojson.NewRecvTxNtfn(txHex, blockDetails(block,
				tx.Index()))
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *nogoutil.Tx,
	clients map[chan struct{}]*wsClient) {

	clientsToNotify := m.subscribedClients(tx, clients)

	if len(clientsToNotify) != 0 {
		n := nogojson.NewRelevantTxAcceptedNtfn(txHexString(tx.MsgTx()))
		marshalled, err := nogojson.MarshalCmd(nogojson.RpcVersion1, nil, n)
		if err != nil {
...
func (m *wsNotificationManager) notifyForTxIns(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	tx *nogoutil.Tx, block *nogoutil.Block) {
	...
	for {
		...
		if !batchedRequest {
			...
			if err != nil {
				...
				jsonErr := &nogojson.RPCError{
					Code:    nogojson.ErrRPCParse.Code,
					Message: "Failed to parse request: " + err.Error(),
				}
				reply, err = createMarshalledReply(nogojson.RpcVersion1, nil, nil, jsonErr)
				if err != nil {
			...
			if req.Method == "" || req.Params == nil {
				jsonErr := &nogojson.RPCError{
					Code:    nogojson.ErrRPCInvalidRequest.Code,
			...
			switch authCmd, ok := cmd.cmd.(*nogojson.AuthenticateCmd); {
			...
			if !c.isAdmin {
				if _, ok := rpcLimited[req.Method]; !ok {
					jsonErr := &nogojson.RPCError{
						Code:    nogojson.ErrRPCInvalidParams.Code,
		...
		if batchedRequest {
			...
			if err != nil {
				...
				reply, err = nogojson.MarshalResponse(nogojson.RpcVersion2, nil, nil, jsonErr)
				if err != nil {
...
func handleWebsocketHelp(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*nogojson.HelpCmd)
	if !ok {
		return nil, nogojson.ErrRPCInternal
	}
	...
	if !valid {
		return nil, &nogojson.RPCError{
			Code:    nogojson.ErrRPCInvalidParameter,
...
func handleLoadTxFilter(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*nogojson.LoadTxFilterCmd)
...
func handleSession(wsc *wsClient, icmd interface{}) (interface{}, error) {
	return &nogojson.SessionResult{SessionID: wsc.sessionID}, nil
}
...
func handleNotifySpent(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*nogojson.NotifySpentCmd)
	if !ok {
		return nil, nogojson.ErrRPCInternal
	}
...
func handleNotifyNewTransactions(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*nogojson.NotifyNewTransactionsCmd)
	if !ok {
		return nil, nogojson.ErrRPCInternal
	}
...
func handleNotifyReceived(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*nogojson.NotifyReceivedCmd)
	if !ok {
		return nil, nogojson.ErrRPCInternal
	}
...
func handleStopNotifySpent(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*nogojson.StopNotifySpentCmd)
	if !ok {
		return nil, nogojson.ErrRPCInternal
	}
...
func handleStopNotifyReceived(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*nogojson.StopNotifyReceivedCmd)
	if !ok {
		return nil, nogojson.ErrRPCInternal
	}
...
func checkAddressValidity(addrs []string, params *chaincfg.Params) error {
	for _, addr := range addrs {
		...
		if err != nil {
			return &nogojson.RPCError{
				Code: nogojson.ErrRPCInvalidAddressOrKey,
...
func deserializeOutpoints(serializedOuts []nogojson.OutPoint) ([]*wire.OutPoint, error) {
...
var ErrRescanReorg = nogojson.RPCError{
	Code:    nogojson.ErrRPCDatabase,
	Message: "Reorganize",
}

// rescanBlock rescans all transactions in a single block.  This is a helper
// function for handleRescan.
func rescanBlock(wsc *wsClient, lookups *rescanKeys, blk *nogoutil.Block) {
	for _, tx := range blk.Transactions() {
		...
		for txOutIdx, txout := range tx.MsgTx().TxOut {
			...
			for _, addr := range addrs {
				...
				ntfn := nogojson.NewRecvTxNtfn(txHex,
...
//
// NOTE: This extension is ported from github.com/decred/dcrd
func rescanBlockFilter(filter *wsClientFilter, block *nogoutil.Block, params *chaincfg.Params) []string {
...
func handleRescanBlocks(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*nogojson.RescanBlocksCmd)
	if !ok {
		return nil, nogojson.ErrRPCInternal
	}
	...
	discoveredData := make([]nogojson.RescannedBlock, 0, len(blockHashes))
	...
	for i := range blockHashes {
		...
		if err != nil {
			return nil, &nogojson.RPCError{
				Code:    nogojson.ErrRPCBlockNotFound,
		...
		if len(transactions) != 0 {
			discoveredData = append(discoveredData, nogojson.RescannedBlock{
...
func descendantBlock(prevHash *chainhash.Hash, curBlock *nogoutil.Block) error {
...
func scanBlockChunks(wsc *wsClient, cmd *nogojson.RescanCmd, lookups *rescanKeys, minBlock,
	maxBlock int32, chain *blockchain.BlockChain) (
	*nogoutil.Block, *chainhash.Hash, error) {
...
fetchRange:
	for minBlock < maxBlock {
		...
		if err != nil {
			rpcsLog.Errorf("Error looking up block range: %v", err)
			return nil, nil, &nogojson.RPCError{
				Code:    nogojson.ErrRPCDatabase,
				Message: "Database error: " + err.Error(),
			}
		}
	...
	loopHashList:
		for i := range hashList {
			...
			n := nogojson.NewRescanProgressNtfn(
				hashList[i].String(), blk.Height(),
				blk.MsgBlock().Header.Timestamp.Unix(),
			)
			mn, err := nogojson.MarshalCmd(nogojson.RpcVersion1, nil, n)
			if err != nil {
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*nogojson.RescanCmd)
	if !ok {
		return nil, nogojson.ErrRPCInternal
	}
	...
	// been sent.
	n := nogojson.NewRescanFinishedNtfn(
		lastBlockHash.String(), lastBlock.Height(),
		lastBlock.MsgBlock().Header.Timestamp.Unix(),
	)
	if mn, err := nogojson.MarshalCmd(nogojson.RpcVersion1, nil, n); err != nil {
