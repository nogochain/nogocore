...
import (
	...
	"github.com/nogochain/nogocore/blockchain/internal/testhelper"
...
// nogoutil.Block.  This is largely borrowed from the test code in btcdb.
func loadBlocks(filename string) (blocks []*nogoutil.Block, err error) {
	...
	for height := int64(1); err == nil; height++ {
		...
		block, err = nogoutil.NewBlockFromBytes(rbytes)
		if err != nil {
			return
		}
...
func addBlock(chain *BlockChain, prev *nogoutil.Block, spends []*testhelper.SpendableOut) (
	*nogoutil.Block, []*testhelper.SpendableOut, error) {
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*nogoutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, nogoutil.NewTx(tx))
	}
...
func newBlock(chain *BlockChain, prev *nogoutil.Block,
	spends []*testhelper.SpendableOut) (*nogoutil.Block, []*testhelper.SpendableOut, error) {
	...
	// SolveBlock.
	block := nogoutil.NewBlock(&wire.MsgBlock{
		Header: wire.BlockHeader{
			Version:    4,
			PrevBlock:  *prev.Hash(),
