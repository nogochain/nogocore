...
//
// This function is safe for concurrent access.
func (b *BlockChain) ProcessBlock(block *nogoutil.Block, flags BehaviorFlags) (bool, bool, error) {
	b.chainLock.Lock()
	...
	if checkpointNode != nil {
		...
		if !fastAdd {
			...
			requiredTarget := nogopow.CompactToBig(b.calcEasiestDifficulty(
				checkpointNode.bits, duration))
			currentTarget := nogopow.CompactToBig(blockHeader.Bits)
...
// can have given starting difficulty bits and a duration. NogoPow does not
// use SHA-256 difficulty retargeting, so this returns PowLimitBits as the
