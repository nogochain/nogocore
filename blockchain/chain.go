...
// exceeded.
func (b *BlockChain) addOrphanBlock(block *nogoutil.Block) {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) CalcSequenceLock(tx *nogoutil.Tx, utxoView *UtxoViewpoint, mempool bool) (*SequenceLock, error) {
	b.chainLock.Lock()
...
func (b *BlockChain) calcSequenceLock(node *blockNode, tx *nogoutil.Tx, utxoView *UtxoViewpoint, mempool bool) (*SequenceLock, error) {
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBlock(node *blockNode, block *nogoutil.Block,
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) disconnectBlock(node *blockNode, block *nogoutil.Block, view *UtxoViewpoint) error {
...
func countSpentOutputs(block *nogoutil.Block) int {
...
func (b *BlockChain) verifyReorganizationValidity(detachNodes, attachNodes *list.List) (
	[]*nogoutil.Block, []*nogoutil.Block, [][]SpentTxOut, error) {
	...
	detachBlocks := make([]*nogoutil.Block, 0, detachNodes.Len())
	detachSpentTxOuts := make([][]SpentTxOut, 0, detachNodes.Len())
	attachBlocks := make([]*nogoutil.Block, 0, attachNodes.Len())
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) connectBestChain(node *blockNode, block *nogoutil.Block, flags BehaviorFlags) (bool, error) {
	...
	if parentHash.IsEqual(&b.bestChain.Tip().hash) {
		...
		if !fastAdd {
			...
			//
			// Validation is performed before connecting each transaction within
			// checkConnectBlock. Passing the UTXO cache directly would avoid the
			// memory allocation from fetchInputUtxos and is planned for a
...
type IndexManager interface {
	...
	// required.
	ConnectBlock(database.Tx, *nogoutil.Block, []SpentTxOut) error

	// DisconnectBlock is invoked when a block has been disconnected from
	// the main chain. The set of outputs scripts that were spent within
	// this block is also returned so indexers can clean up the prior index
	// state for this block.
	DisconnectBlock(database.Tx, *nogoutil.Block, []SpentTxOut) error
}
...
func New(config *Config) (*BlockChain, error) {
	...
	if adjustmentFactor == 0 {
		adjustmentFactor = 4 // default retarget adjustment factor
	}
	b := BlockChain{
