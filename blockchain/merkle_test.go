...
func TestMerkle(t *testing.T) {
	block := nogoutil.NewBlock(&Block100000)
...
func makeTxs(size int) []*nogoutil.Tx {
	var txs = make([]*nogoutil.Tx, size)
	for i := range txs {
		tx := nogoutil.NewTx(wire.NewMsgTx(2))
		tx.Hash()
		txs[i] = tx
	}
	return txs
...
func benchmarkRollingMerkle(b *testing.B, txs []*nogoutil.Tx) {
	b.ReportAllocs()
	b.ResetTimer()
...
func benchmarkMerkle(b *testing.B, txs []*nogoutil.Tx) {
	b.ResetTimer()
	b.ReportAllocs()
