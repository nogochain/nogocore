...
func isNonstandardTransaction(tx *nogoutil.Tx) bool {
...
// scriptClassType represents the classification of a public key script.
// NogoCore has removed the Bitcoin script engine; all scripts are treated
// as non-standard for classification purposes.
type scriptClassType int

// nonStandardTy indicates a non-standard script type. This is the only
// classification used since the Bitcoin script engine has been removed.
const nonStandardTy scriptClassType = 0

// getScriptClass returns the script class for the given public key script.
// Since NogoCore does not implement a Bitcoin script engine, all scripts
// are classified as non-standard.
func getScriptClass(pkScript []byte) scriptClassType {
...
//
// This function is safe for concurrent access.
func (b *BlockChain) IsCheckpointCandidate(block *nogoutil.Block) (bool, error) {
	b.chainLock.RLock()
