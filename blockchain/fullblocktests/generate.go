...
func makeTestGenerator(params *chaincfg.Params) (testGenerator, error) {
	privKey, _ := nogoec.PrivKeyFromBytes([]byte{0x01})
...
func additionalCoinbase(amount nogoutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee nogoutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
func createSpendTxForTx(tx *wire.MsgTx, fee nogoutil.Amount) *wire.MsgTx {
...
func Generate(includeLargeReorg bool) (tests [][]TestInstance, err error) {
	...
	for i := int64(0); i < coinbaseMaturity; i++ {
	...
	// requires an unsolved block.
	{
		...
		for {
			...
			hashNum := testhelper.HashToBigStub(&blockHash)
