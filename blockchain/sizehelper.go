...
func calculateRoughMapSize(hint int, bucketSize uintptr) int {
	// This code is adapted from makemap() in runtime/map.go.
	// The Go runtime may change the map bucket sizing algorithm
	// in future releases; this implementation is verified against
	// Go 1.24 and should be re-validated on major Go version bumps.
	...
	// This code is adapted from makeBucketArray() in runtime/map.go.
	// Re-validate against the current Go runtime on major version updates.
	//
