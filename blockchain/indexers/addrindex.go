...
type AddrIndex struct {
	...
	txnsByAddr      map[[addrKeySize]byte]map[chainhash.Hash]*nogoutil.Tx
...
func (idx *AddrIndex) indexPkScript(data writeIndexData, pkScript []byte, txIdx int) {
	...
	_, addrs, _, err := extractPkScriptAddrs(pkScript,
		idx.chainParams)
...
// the passed map.
func (idx *AddrIndex) indexBlock(data writeIndexData, block *nogoutil.Block,
...
//
// This is part of the Indexer interface.
func (idx *AddrIndex) ConnectBlock(dbTx database.Tx, block *nogoutil.Block,
...
//
// This is part of the Indexer interface.
func (idx *AddrIndex) DisconnectBlock(dbTx database.Tx, block *nogoutil.Block,
	stxos []blockchain.SpentTxOut) error {
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) indexUnconfirmedAddresses(pkScript []byte, tx *nogoutil.Tx) {
	// The error is ignored here since the only reason it can fail is if the
	// script fails to parse and it was already validated before being
	// admitted to the mempool.
	_, addresses, _, _ := extractPkScriptAddrs(pkScript,
		idx.chainParams)
	for _, addr := range addresses {
		...
		if addrIndexEntry == nil {
			addrIndexEntry = make(map[chainhash.Hash]*nogoutil.Tx)
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) AddUnconfirmedTx(tx *nogoutil.Tx, utxoView *blockchain.UtxoViewpoint) {
...
//
// This function is safe for concurrent access.
func (idx *AddrIndex) UnconfirmedTxnsForAddress(addr address.Address) []*nogoutil.Tx {
	// Ignore unsupported address types.
	addrKey, err := addrToKey(addr)
	if err != nil {
		return nil
	...
	if txns, exists := idx.txnsByAddr[addrKey]; exists {
		addressTxns := make([]*nogoutil.Tx, 0, len(txns))
		for _, tx := range txns {
...
func NewAddrIndex(db database.DB, chainParams *chaincfg.Params) *AddrIndex {
	return &AddrIndex{
		...
		txnsByAddr:  make(map[[addrKeySize]byte]map[chainhash.Hash]*nogoutil.Tx),
...
// extractPkScriptAddrs extracts addresses from a standard public key script
// using pattern matching on script byte templates. Supports P2PKH, P2SH,
// P2WPKH (native segwit v0), P2WSH, and P2TR (taproot). Returns the script
// class, extracted addresses, required signatures, and any error.
func extractPkScriptAddrs(script []byte, params *chaincfg.Params) (int, []address.Address, int, error) {
	if len(script) == 0 {
		return 0, nil, 0, nil
	}
	...
	if len(script) == 25 && script[0] == 0x76 && script[1] == 0xa9 &&
		...
		return 1, []address.Address{addr}, 1, nil
	}
	...
	if len(script) == 23 && script[0] == 0xa9 && script[1] == 0x14 &&
		script[22] == 0x87 {
		hash := script[2:22]
		addr, err := address.NewAddressScriptHash(hash, params)
		if err != nil {
			return 0, nil, 0, err
		}
		return 2, []address.Address{addr}, 1, nil
	}

	// P2WPKH (native segwit v0): OP_0 <20-byte key hash>
	//        0x00 0x14 <20 bytes>
	if len(script) == 22 && script[0] == 0x00 && script[1] == 0x14 {
		hash := script[2:22]
		addr, err := address.NewAddressWitnessPubKeyHash(hash, params)
		if err != nil {
			return 0, nil, 0, err
		}
		return 3, []address.Address{addr}, 1, nil
	}

	// P2WSH (native segwit v0): OP_0 <32-byte script hash>
	//        0x00 0x20 <32 bytes>
	if len(script) == 34 && script[0] == 0x00 && script[1] == 0x20 {
		hash := script[2:34]
		addr, err := address.NewAddressWitnessScriptHash(hash, params)
		if err != nil {
			return 0, nil, 0, err
		}
		return 4, []address.Address{addr}, 1, nil
	}
	...
	if len(script) == 34 && script[0] == 0x51 && script[1] == 0x20 {
		hash := script[2:34]
		addr, err := address.NewAddressTaproot(hash, params)
		if err != nil {
			return 0, nil, 0, err
		}
		return 5, []address.Address{addr}, 1, nil
	}

	// OP_RETURN (null data): 0x6a ...
	if len(script) > 0 && script[0] == 0x6a {
		return 0, nil, 0, nil
	}

	// Non-standard or unrecognized script; no addresses to extract.
	return 0, nil, 0, nil
}
