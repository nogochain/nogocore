...
func TestHaveBlock(t *testing.T) {
	...
	_, isOrphan, err := chain.ProcessBlock(nogoutil.NewBlock(&Block100000),
		BFNone)
	if err != nil {
...
func TestCalcSequenceLock(t *testing.T) {
	...
	numBlocksToActivate := int32(netParams.MinerConfirmationWindow * 3)
	for i := int32(0); i < numBlocksToActivate; i++ {
	...
	// age of 4 blocks.
	targetTx := nogoutil.NewTx(&wire.MsgTx{
		TxOut: []*wire.TxOut{{
			PkScript: nil,
			Value:    10,
		}},
	...
	utxoView.AddTxOuts(nogoutil.NewTx(unConfTx), 0x7fffffff)
...
func addBlocks(count int, chain *BlockChain, prevBlock *nogoutil.Block,
	...
	tests := []struct {
	...
	}{
		{
			...
			chainGen: func() (*BlockChain, []*chainhash.Hash, func()) {
				...
				tip := nogoutil.NewBlock(params.GenesisBlock)
				tip.SetHeight(0)
