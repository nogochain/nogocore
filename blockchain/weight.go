...
func GetBlockWeight(blk *nogoutil.Block) int64 {
...
func GetTransactionWeight(tx *nogoutil.Tx) int64 {
	msgTx := tx.MsgTx()
...
func GetSigOpCost(tx *nogoutil.Tx, isCoinBaseTx bool, utxoView *UtxoViewpoint, bip16, segWit bool) (int, error) {
	...
	if segWit && !isCoinBaseTx {
		...
		for txInIndex, txIn := range msgTx.TxIn {
			...
			_ = txIn.Witness
			_ = txIn.SignatureScript
			_ = utxo.PkScript()
			// GetWitnessSigOpCount removed per NogoCore
		}
